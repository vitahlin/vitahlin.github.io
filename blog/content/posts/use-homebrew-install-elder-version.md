...
克隆[Homebrew-code](https://github.com/Homebrew/homebrew-core)，查看`gradle`指定版本对应的提交记录：
```bash
...
如果近期不打算更新`gradle`，可以用`pin`命令把`gradle`“钉“在当前版本，防止不小心运行`brew upgrade`命令导致的更新：
```bash
$ brew pin gradle
```  
